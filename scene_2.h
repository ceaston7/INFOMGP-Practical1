...
public:
    ...
    void updateImpulseVelocities(){
              ...
              for (int i=0;i<impulses.size();i++){
                  ...
                  RowVector3d angImp = invIT*(pos.cross(dir).transpose());
                  RowVector3d linImp = (dir)/(mass/1000);
    ...
    void updateVelocity(double timeStep, double airDrag){
      ...
      //   comVelocity = (comVelocity + PVelocity)*timeStep;
         ...
         cout << "comVelocity before updating: "<<comVelocity<<endl;
        /* for(int i=0; i<currV.rows(); i++){
           r.row(i) = currV.row(i)-COM;
         }
         for(int i=0; i<r.rows(); i++)
         {
           rtemp = r.row(i);
         ...
         //cout << "PVelocity: "<<PVelocity<<endl;
         comVelocity = (gravity*timeStep) + comVelocity;
         comVelocity -= airDrag*comVelocity;
         cout <<"comVelocity after updating: "<<comVelocity<<endl;
    }
...
public:
    ...
    void handleCollision(RigidObject& ro1, RigidObject& ro2, const double depth, const RowVector3d& contactNormal, const RowVector3d& penPosition, const double CRCoeff, double friction){
      ...
      //  RowVector3d ro1TotalClosingVelocity = ro1.comVelocity+ro1.angVelocity.cross(ro1R);
    //    RowVector3d ro2TotalClosingVelocity = ro2.comVelocity+ro2.angVelocity.cross(ro2R);
      //  double impulseMagNumerator = -(1+CRCoeff)*((ro1TotalClosingVelocity-ro2TotalClosingVelocity).dot(contactNormal));
    //    const double abcde = ro1RCrossNorm.transpose()*ro1.invIT*ro1RCrossNorm;
      //  double sumOfMatrices = (ro1RCrossNorm.transpose()*ro1.invIT*ro1RCrossNorm)+(ro2RCrossNorm.transpose()*ro2.invIT*ro1RCrossNorm);
  //      double impulseMagDenominator = ((1/ro1.mass)+(1/ro2.mass)/*+(((ro1RCrossNorm.transpose()*ro1.invIT)*(ro1RCrossNorm)).dot((ro2RCrossNorm.transpose()*ro2.invIT)*(ro1RCrossNorm)))*/);
    //    double impulseMag = impulseMagNumerator/impulseMagDenominator;
      //  double impulseMag = ((1/ro1.mass)+(1/ro2.mass)+(((ro1R.cross(contactNormal).transpose())*(ro1.invIT)*(ro1R.cross(contactNormal)))+((ro2R.cross(contactNormal).transpose())*(ro2.invIT)*(ro2R.cross(contactNormal)))));
    ...
    //   RowVector3d frictionTan = (contactNormal.cross(ro1.comVelocity - ro2.comVelocity)).cross(contactNormal);
       //frictionTan.normalize();
       //frictionTan *= friction;
